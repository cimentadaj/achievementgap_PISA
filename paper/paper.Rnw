...
# results_math <- test_diff(adapted_year_data, reliability_pisa, "MATH", c(0.25, 0.75))
# results_read <- test_diff(adapted_year_data, reliability_pisa, "READ", c(0.25, 0.75))
# results_math_topmid <- test_diff(adapted_year_data, reliability_pisa, "MATH", c(0.5, 0.75))
# results_read_topmid <- test_diff(adapted_year_data, reliability_pisa, "READ", c(0.5, 0.75))
# results_math_midbottom <- test_diff(adapted_year_data, reliability_pisa, "MATH", c(0.25, 0.5))
# results_read_midbottom <- test_diff(adapted_year_data, reliability_pisa, "READ", c(0.25, 0.5))

# results_math_75 <- results_math
# results_read_75 <- results_read
# results_math_topmid_75 <- results_math_topmid
# results_read_topmid_75 <- results_read_topmid
# results_math_midbottom_75 <- results_math_midbottom
# results_read_midbottom_75 <- results_read_midbottom
...
\begin{equation}
\gamma_i = \alpha_{j[i]} + \beta_{j[i]} * SES_i + \epsilon_i,\ \text{for} \ i \ \text{= 1, 2, ...,} \ n \ \text{within country} \ j
...
escs_data <- escs_dummy_creator(adapted_year_data, c(0.25, 0.75))
...
rm(sample_tables_topbottom, sample_tables_topmid, sample_tables_midbottom,
   avg_performance, summary_table, table_coming)
@
...
complete_data_topbottom <- mutate(complete_data_topbottom, type = "75th/25th SES gap")
complete_data_topmid <- mutate(complete_data_topmid, type = "75th/50th SES gap")
complete_data_midbottom <- mutate(complete_data_midbottom, type = "50th/25th SES gap")
@
...
p + geom_line(data = complete_data_topbottom %>%
                mutate(country = factor(country, levels = ordered_cnt, ordered = TRUE)) %>%
                filter(!is.na(country)),
              stat = "smooth", method = "lm", aes(group = 1),
...
<<>>=
m <- complete_data_topbottom %>%
  filter(type_test == "math") %>%
  select(wave, country, difference) %>%
  left_join(tracking_data, by = c("country" = "cntry_name")) %>%
  left_join(central_examination) %>%
  mutate(num_tracks = factor(tracks15y))

m %>%
  filter(country %in% countries) %>%
  split(.$country) %>%
  map(~ lm(difference ~ wave + ztrack, data = .x))

p + geom_smooth(data = complete_data_topbottom %>%
                mutate(country = factor(country, levels = ordered_cnt, ordered = TRUE)) %>%
                filter(!is.na(country)),
                method = "lm", se = FALSE,
                aes(group = 1), size = 0.7,
                colour = "red") +
  scale_y_continuous(name = "90/10 gap in SD", expand = c(0, 0), lim = c(0, 3)) +
  scale_x_discrete(name = NULL, breaks = c(2000, 2009, 2015)) +
  ggthemes::theme_few() +
...
<<us_sd_start>>=
us_math_2000 <-
  filter(complete_data_topbottom,
         wave == 2000, country == "United States",
         type_test == "math") %>%
  .[["difference"]]
...
<<deleting_used_data>>=
rm(diff_data, us_math_2000, us_math_2000_upper, us_math_2000_lower)
@
...
rm(full_data, avg_sd_increase_high, avg_sd_increase_low)
@
...
wd_extract <- function(var, name) {
  wdi_data <-
    WDI(indicator = var, start = 2000, end = 2015) %>%
    select(-1) %>%
    as_tibble() %>%
    mutate(year = as.character(year))
  
  names(wdi_data)[2] <- name
  wdi_data
}

pop_preprimary <- wd_extract("SP.PRE.TOTL.IN", "pop_preprimary")
teacher_pupil_ratio <- wd_extract("SE.PRE.ENRL.TC.ZS", "teacher_pupil_ratio")
...
ready_data <-
  ...
  left_join(tracking_data, by = c("country" = "cntry_name")) %>%
  left_join(central_examination) %>%
  mutate(num_tracks = factor(car::recode(tracks15y, "5 = 4")),
         age_selection = ifelse(selage >= 15, 1, 0))

library(rstanarm)

mod_tracking <- stan_glmer(
  difference ~
    num_tracks +
...
#   lmer(difference ~
#        num_tracks +
#        age_selection + (1 | country), data = .)

# new_data <-
#   data_grid(ready_data,
#             selage, num_tracks, central_examination, .model = mod_tracking) %>%
#   filter(complete.cases(.))

new_data <-
  ready_data %>%
  filter(!is.na(age_selection), !is.na(num_tracks)) %>%
...
new_data$pred <- colMeans(predictions)
new_data$pred_low <- new_data$pred - (2 * sd_obs)
new_data$pred_high <- new_data$pred + (2 * sd_obs)
...
# What happened if you adjut the trend splines for the tracking data?
@
...
ready_data_two <-
  ...
  group_by(country) %>%
  summarize(
    cum_diff = sum(difference, na.rm = TRUE),
    length = unique(length),
    num_tracks = unique(num_tracks),
    age_selection = unique(age_selection),
    central_examination = unique(central_examination),
    avg_track = unique(ztrack)
         )
