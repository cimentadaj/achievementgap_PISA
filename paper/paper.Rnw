...
new_data$pred <- predict(mod_tracking, newdata = new_data)

new_data %>%
  ggplot(aes(selage, pred, colour = num_tracks)) +
  geom_point()

new_data %>%
  ggplot(aes(num_tracks, pred, fill = as.character(num_tracks))) +
  geom_col(position = "dodge") +
  facet_wrap(~ central_examination) +
