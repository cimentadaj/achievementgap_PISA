...
# Check whether the previous model can be generalized to a multilevel
# and see how much it can explain between country var
mod_tracking <-
  brms::brm(
    difference ~
      ztrack +
...
# The code bloew needs to be adapted to the model from above (
# this is from previous models)
