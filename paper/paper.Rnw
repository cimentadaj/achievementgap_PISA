...
r_square <-
  paste0("\\hline ", paste0("R square: & ",
         paste0(map(all_gaps_models_cum, ~ bayes_R2(.x)[1] %>% round(2) * 100) %>% paste0("\\%"),
                       collapse = " & ")), " \\\\")
n_obs <-
  paste0(paste0(" Sample size: & ",
         paste0(map(all_gaps_models_cum, nobs), collapse = " & ")), " \\\\ \\hline")

command_to_row <- paste0(r_square, n_obs)

addtorow_two <- list(pos = list(pos = 3), command = command_to_row)
