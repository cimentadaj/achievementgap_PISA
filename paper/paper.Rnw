...
<<creating_data, eval = F>>=
# For now I don't run it, but it should be run when you leave the paper as done.
  
# This file downloads PISA 2012 and PISA 2015 and saves them in ./data/pisa_listcol.Rdata
...
The overaching aim of this paper is to study the evolution of the high/low SES achievement gap in the past 15 years for all PISA participating countries and search for a likely explanation for the evolution.
...
% \begin{figure}
% \begin{center}
<<variance_pisa, include = T, out.height = '3in', out.width = '3.5in', fig.align = 'center', fig.cap = 'Standard deviation of test scores across all waves'>>=
...
% \begin{figure}
% \begin{center}
<<graphing_9010gaps, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Evolution of the achievement gap for selected countries'>>=
...
% \begin{figure}
% \begin{center}
<<graphing_perc_increase, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Evolution of the achievement gap for separate SES groups'>>=
...
% \begin{figure}
% \begin{center}
<<rate_increase/decrease, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Evolution of the achievement gap for separate SES groups'>>=
...
% \begin{figure}
% \begin{center}
<<graphing_allgaps, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Evolution of 90/50/10 SES gaps for selected countries'>>=
...
% \begin{figure}
% \begin{center}
<<rate_change_graph, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Rate at which top/bottom SES groups are catching up for the 90/10 SES gap'>>=
...
% \begin{figure}
% \begin{center}
<<graphing_ses_growth, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Evolution of the SES gaps for top/bottom groups'>>=
...
% \begin{figure}
% \begin{center}
<<avgperformance_inequality, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Relationship between 90/10 achievement gap and average performance in mathematics'>>=
data_to_plot %>%
  ggplot(aes(scale(Mean), difference)) +
...
% \begin{figure}
% \begin{center}
<<country_achievement_and_inequality, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Time trends of achievement gap and average achievement between 2000 and 2015'>>=
data_to_plot %>%
  ...
  ggtitle("Evolution of the relationship between achievement and inequality") +
  facet_wrap(~ country, ncol = 5) +
  theme_few() +
  theme(panel.spacing = unit(1, "lines"),
        legend.position = "bottom")
...
<<complete_tracking_model, results = 'hide'>>=
all_track_models <-
  stan_model_builder(
  dv = "difference",
...
<<complete_tracking_results, include = TRUE>>=
all_track_models %>%
  stan_table() %>%
  mutate(" " = c(
    "Only 1 track",
...
<<graph_complete_tracking_model, results = 'hide'>>=
...
<<graph_complete_tracking_graph, include = T, fig.height = 9, fig.width = 9, out.height = '7in', out.width = '6.5in', fig.align = 'center', fig.cap = 'Simulation: predicted 90/10 achievement gap for different tracking setups'>>=
new_data <-
  ready_data_age %>%
...
<<counterfactual_tracking, include = TRUE>>=
pred_avg_diff <-
  new_data %>%
  left_join(sum_ready_data) %>%
...
track_should_not_change <- setdiff(pred_avg_diff$country, track_should_change)
...
<<other_counterfactual, eval = F>>=

pred_avg_diff <-
  new_data %>%
  left_join(sum_ready_data) %>%
  filter(!is.na(avg_diff) | as.numeric(num_tracks) > 1 & age_selection == 0) %>%
  select(country, num_tracks, age_selection, pred, avg_diff)

avg_increase <-
  pred_avg_diff %>%
  filter(country %in% track_should_not_change) %>%
  fill(avg_diff, .direction = "up") %>%
  group_by(country, num_tracks = ifelse(as.numeric(num_tracks) > 1, "more", "one")) %>%
  summarize(pred = mean(pred, na.rm = TRUE),
            avg_diff = mean(avg_diff, na.rm = TRUE)) %>%
  filter(num_tracks == "more") %>%
  mutate(reduction = ((pred - avg_diff) / avg_diff) * 100) %>%
  pull(reduction) %>%
  mean
...
<<interaction_model, results = 'hide'>>=
...
<<interaction_table,  include = TRUE>>=
stan_models %>%
  stan_table() %>%
  mutate(" " = c(
    "Tracking Index",
...
sig(0.41, , -0.22, ) # the two arguments missing are the SE's of both coefficients.
# take from the model from above and finish calculate (already did before, they're sig different from each other but I don't want to rerun again! :P)

@

% \begin{figure}[!htbp]
% \begin{center}
<<interaction_plot, include = T, fig.width = 8, out.height = '5in', out.width = '6.5in', fig.align = 'center', fig.cap = 'Interaction between tracking and vocational values. Legend ordered relative to the lines'>>=
plot_interaction <-
...
data_interaction <-
  plot(plot_interaction, plot = FALSE)[[1]] %>% ggplot_build() %>% pluck(1) %>% pluck(1)
...
% \begin{figure}[!htbp]
% \begin{center}
<<all_gaps_plot, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'What explains achievement gaps? Model comparisons'>>=
model_data_merged <-
...
<<cumulative_modeling, results = 'hide'>>=
...
vars_unique <- c("length", "num_tracks", "age_selection", "ztrack",
                 "zvoc")
...
<<cumulative_modeling_table, include = TRUE>>=
all_gaps_models_cum %>%
  stan_table() %>%
  mutate(" " = c(
    "Track Index",
...
<<graph_all_gaps, include = T, fig.pos = '!htbp', out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Evolution of the achievement gap for several gaps'>>=
complete_gaps %>%
  mutate(country = factor(country, levels = ordered_cnt, ordered = TRUE)) %>%
  filter(type_test == "math",
...
<<percentage_change_8020,  include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Percentage change in the 80/20 achievement gap from 2000 to 2015'>>=
...
<<percentage_change_7030,  include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center', fig.cap = 'Percentage change in the 70/30 achievement gap from 2000 to 2015'>>=
...
m_data <-
  ...
  mutate(pred_median = posterior_predict(all_gaps_models_cum[[1]]) %>% colMeans(),
