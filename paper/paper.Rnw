...
us_math_2000_upper <-
  ...
  .[["upper"]]
@
We find that the initial gap for the U.S in 2000 is \Sexpr{us_math_2000} varying between \Sexpr{us_math_2000_lower} and \Sexpr{us_math_2000_upper}. This gap is much higher than found in previous studies, but again, we don't know much previous estimates vary and nobody has really studied the 15-year old achievement gap precisely. All country-level studies pool many studies together and control for age, removing the age-specific gap effect.

Figure 2 below takes a more direct approach and looks at the percentage change from the first and last timepoint available. Each data point has been computed together with it's 50\% and 95\% uncertainty interval \footnote{Each of these uncertainty intervals were computed using a 500-replicate bootstrap}.
...
complete_data_topbottom %>%
  ...
  mutate(type = factor(type,
                       ...
                       ordered = TRUE)) %>%
  ggplot(aes(as.factor(wave), difference, group = type_test, colour = type_test, shape = type_test)) +
...
\begin{figure}
\begin{center}
<<avgperformance_inequality, include = T, out.height = '5in', out.width = '7in', fig.align = 'center'>>=
data_to_plot %>%
  ggplot(aes(scale(Mean), difference)) +
...
data_to_plot %>%
  ...
  filter(country %in% countries) %>%
  ggplot(aes(as.character(wave), value, group = type, colour = type)) +
