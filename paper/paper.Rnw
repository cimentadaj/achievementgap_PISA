...
% Make country summaries: Like the US is decreaing but because of bla bla (summing up everything up until now). Look at the graph and build rough summaries.
...
These results actually confirm that there exists a partial ecological fallacy. We see countries like Poland, Canada and Germany where the pattern is just as the aggregate trend, namely that as the average performance increases, inequality seems to be reduced in parallel. Conversely, France, Austria and Australia, to name a few, portray the opposite: as average performance decreases inequality increases. With these results we confirm our second hypothesis, that there is negative relationship. But we also unconver important variation within countries in the relationship between average achievement and dispersion of achievement.
...
<<graph_complete_tracking_graph, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center'>>=
new_data <-
  ready_data_age %>%
...
The red point for Germany is in the '\textgreater 1 track' and '\textless 15 age' setup, meaning that Germany has a tracking system that has more than 1 track and an age of selection of below 15. We see that the model predicts the level of inequality for that set up quite accurately, relative to the actual level (red point). But if the setup would be in the ideal '1 track' and age of selection above or equal to 15, the inequality would be much lower. Generally speaking, this applies to all countries in the plot. Virtually for all countries which are not in this 'ideal' tracking, a switch to the 'ideal' would reflect a reduction of the achievement gap.

% Comment on the above graph for particular countries! Make some observations

For those countries which are not in the 'ideal' tracking setup, we compute below the percentage change from switching to the ideal tracking.
...
<<interaction_plot, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center'>>=
plot_interaction <-
...
% Talk about the mechanisms why tracking might be explaining this, although tracking doesn't change over time.

% Because it might've caused it at the beginning and now it just exarcebates it.

% Because when combined with vocational enrollment, it might be exarcebarting even more, if enrollment is not high.
...
<<all_gaps_plot, include = T, out.height = '5in', out.width = '5.5in', fig.align = 'center'>>=
model_data_merged <-
